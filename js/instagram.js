...
function toonBestellingPopup(kaartnaam, prijs, gepersonaliseerd) {
  const prijsLabel = gepersonaliseerd
    ? `${formatEuro(PRIJZEN.GEPERSONALISEERD)} (gepersonaliseerd)`
    : formatEuro(prijs);

  const bericht = gepersonaliseerd
    ? `Hoi Rosalie! Ik wil graag een gepersonaliseerde kaart bestellen. Kunnen we samen kijken naar een ontwerp dat past bij mijn idee? (${formatEuro(PRIJZEN.GEPERSONALISEERD)})`
  ...
  overlay.innerHTML = `
    <div class="dm-popup">
      ...
      <p class="dm-uitleg">Kopieer dit bericht en stuur het via Instagram DM naar <strong>@${INSTAGRAM_HANDLE}</strong>. Verzendkosten hoor je bij de bevestiging, gratis vanaf ${formatEuro(VERZENDING.GRATIS_VANAF)}.</p>
